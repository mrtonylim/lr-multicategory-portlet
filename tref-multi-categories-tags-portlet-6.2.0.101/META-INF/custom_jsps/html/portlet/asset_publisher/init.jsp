...
String trefCategoryId = ParamUtil.getString(request, "categoryId", StringPool.BLANK);

String[] trefCategoryIds = StringUtil.split(trefCategoryId);

if(trefCategoryIds.length == 1) {
  
	 assetCategoryId = Long.parseLong(trefCategoryIds[0]);
	
	 assetCategoryTitle = null;
	 assetVocabularyTitle = null;
...
} else if (trefCategoryIds.length > 1) {
  
  	assetCategoryId = Long.parseLong(trefCategoryIds[0]);
  
	List<Long> categoryIdsList = new ArrayList<Long>();
	
	for(String categoryIdString : trefCategoryIds) {
...
String assetTagName = null;

String trefAssetTagName = ParamUtil.getString(request, "tag");

String[] trefAssetTagNames = StringUtil.split(trefAssetTagName);

if (trefAssetTagNames.length > 0) {
  
  	assetTagName = trefAssetTagNames[0];
  
	allAssetTagNames = trefAssetTagNames;
	...
	for (String tagName : trefAssetTagNames) {

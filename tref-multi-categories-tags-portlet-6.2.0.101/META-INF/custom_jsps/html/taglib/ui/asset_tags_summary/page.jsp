...
<%-- INIZIO HOOK --%>
<%
String tagNamesString = ParamUtil.getString(request, "tag", StringPool.BLANK);
String[] tagNames = StringUtil.split(tagNamesString);
%>
<%-- FINE HOOK --%>
		...
		<c:choose>
			<c:when test="<%= portletURL != null %>">
				...
				for (int i = 0; i < assetTagNames.length; i++) {
				  	/* INIZIO HOOK */
					String[] paramTags = tagNames;
					if (!ArrayUtil.contains(paramTags, assetTagNames[i])) {
					  paramTags = ArrayUtil.append(paramTags, assetTagNames[i]);
					}
					portletURL.setParameter("tag", StringUtil.merge(paramTags));
					/* FINE HOOK */
				%>

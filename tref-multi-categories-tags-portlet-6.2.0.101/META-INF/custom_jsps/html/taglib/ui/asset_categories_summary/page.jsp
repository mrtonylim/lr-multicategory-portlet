...
<%-- INIZIO HOOK --%>
<%
String categoryIdsString = ParamUtil.getString(request, "categoryId", StringPool.BLANK);
String[] categoryIds = StringUtil.split(categoryIdsString);
%>
<%-- FINE HOOK --%>
			...
			<c:choose>
				<c:when test="<%= portletURL != null %>">
					...
					for (AssetCategory category : curCategories) {
						...
						/* INIZIO HOOK */
						String[] paramCategoryIds = categoryIds;
						if (!ArrayUtil.contains(paramCategoryIds, String.valueOf(category.getCategoryId()))) {
							paramCategoryIds = ArrayUtil.append(paramCategoryIds, String.valueOf(category.getCategoryId()));
						}
						portletURL.setParameter("categoryId", StringUtil.merge(paramCategoryIds));
						/* FINE HOOK */
					%>
